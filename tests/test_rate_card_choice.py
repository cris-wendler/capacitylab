...
FAMILY = {f"db.r6g.{s}": p for s, p in (("large", 0.6), ("xlarge", 1.2), ("2xlarge", 2.4), ("4xlarge", 4.8),
                                         ("8xlarge", 9.6), ("12xlarge", 14.4), ("16xlarge", 19.2))}
...
def test_aws_prices_are_used_when_they_cover_every_needed_class():
    ...
    assert {"db.r6g.2xlarge", "db.r6g.4xlarge"} <= needed
    ...
    assert choice.card.hourly("db.r6g.4xlarge") == 4.8 and choice.card.storage_gib_month == 0.10
...
def test_partial_aws_prices_fall_back_and_say_what_is_missing():
    scenario, bundle = load_scenario("campaign-overlap")
    partial = {k: v for k, v in FAMILY.items() if k != "db.r6g.4xlarge"}
    ...
    assert choice.missing_classes == ["db.r6g.4xlarge"] and "do not cover db.r6g.4xlarge" in choice.note
