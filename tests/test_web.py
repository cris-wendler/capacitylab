...
def test_what_if_options_recompute_on_the_server(client):
    ...
    assert "7<small>/10</small>" in page.text and "$304,500" in page.text and "$18,708.48 one-off" in page.text
    ...
    assert lower.status_code == 200 and "10<small>/10</small>" in lower.text and "<svg class=\"viz\"" in lower.text
    assert 'risk-off' in lower.text
    higher = client.get("/scenarios/campaign-overlap/options", params={"A-CAMPAIGN-MULT": "6"})
    assert "4<small>/10</small>" in higher.text and "0 of them cost nothing" in higher.text and "$522,000" in higher.text
